...
def _attach_classification_head(base_model, tensors: dict):
    ...
    if not hasattr(base_model, "score"):
        ...
        if current_out_features != num_labels or current_in_feature != hidden_size:
            ...
            logger.info(
                f"Replacing the existing shape of {current_out_features}, {current_in_feature} with {num_labels, hidden_size}")
            from vllm.model_executor.layers.linear import RowParallelLinear
            base_model.score = RowParallelLinear(hidden_size, num_labels, bias=True, prefix="").to(device,
...
class LoRAModel(AdapterModel):
    ...
    def from_lora_tensors(
        ...
        loras = {module_name: weights for module_name, weights in loras.items() if
                 module_name not in classification_keys}
